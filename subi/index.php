...
if($event == "upload_foto") {
	...
	$tplmarco->Open("/upload_foto.html");
...
sleep(1);
/*
if(($event == "imprimir") && ($idficha > 0)){



	$tplmarco = new Template("");
	$tplmarco->setFileRoot($TEMPLATE_ROOT);
	$tplmarco->Open("/home_ficha_imprimir_es.html");
	...
	foreach( $a->fetchall() as $row)
	{
		while ($tplmarco->isTag("<!-- BEGIN $row[descripcion] -->"))
		{
			...
			$tplmarco->setVar("<!-- BLOCK $row[descripcion] -->",
				getArea_by_Id(clone($db), $row[idarea], $tplarea->Template, "Nota", $area_destacada, $area_cantidad, "", "publicacion.fecha_publicacion DESC, publicacion.orden DESC", $area_paginar)
			);
		}
	}
	...
	$objFicha = new ficha();
	$objFicha->db = $db;
	$objFicha->fetch($idficha);

	$result = $objFicha->doformat($tplmarco->Template);
