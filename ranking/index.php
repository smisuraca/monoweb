...
//{
	...
	$sql = "SELECT DISTINCT contenidos.idcontenido as id FROM contenidos, publicacion WHERE publicacion.idcontenido = contenidos.idcontenido AND contenidos.idcontenido_tipo = 1 AND extra3 > 5 ORDER BY extra4 desc, extra1 desc LIMIT 3";
	$db->exec($sql);
	...
	$sql = "SELECT DISTINCT contenidos.idcontenido as id FROM contenidos, publicacion WHERE publicacion.idcontenido = contenidos.idcontenido AND contenidos.idcontenido_tipo = 1 AND extra3 > 5 ORDER BY extra4 desc, extra1 desc LIMIT 3,7";
	$db->exec($sql);
	$strResultado = "";
	...
	$tplmarco->setVar('{pagina_descripcion}', "Ranking de Cinevivo, los cortos mas votados por la gente");	
