...
*/
	...
	define(CACHE_DIR, "/tmp/phpCache");	/* Default: /tmp/phpCache/ - Default cache directory */
	...
	define(CACHE_MAX_FILENAME_LEN, 50);	/* How long the cache storage filename can be before it will md5() the entire thing */
	...
	function cache($cachetime, $object, $key=NULL) {
		...
		$curtime=time();
		cache_debug("Caching based on <b>OBJECT</b>=$object <b>KEY</b>=$key");
		...
		if (($buff=cache_read($cache_absfile))) {
			...
			cache_debug("Failed to open previous cache of $cache_absfile");
		}
		$oldum=umask();
		umask(0077);
		...
		/* If ob_get_length() returns false, output buffering was not on.  turn it on. */
			if (cache_iftype($cache_pbufferlen,FALSE)) ob_start();
		} else {
			$cache_pbufferlen=FALSE;
		}
		return 0;
	...
	/* This *MUST* at the end of a cache() block or else the cache
		will not be stored! */ 
