...
//{
	...
	$objUsuarios->db = $db;
	$objUsuarios->where="username like '".$username."' OR email like '".$mail."'";
	if($objUsuarios->fetchall() || (!$nombre) || (!$apellido) || (!$mail)) {
		...
		$objUsuarios->field("password", substr(md5($mail.$nombre.$apellido.date("Y-m-d h:i:s")),0,8));
...
function enviaMail($idusuario) {
	...
	$objContenido->db = $db;
	$objContenido->where = "contenidos_tipos.descripcion = 'Cinevivo::Mailregistro'";
